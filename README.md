...
| 変数名 | 用途 | 例 |
| --- | --- | --- |
| `PUBLIC_SITE_URL` | canonical / OGP / sitemap の基準URL。未設定時は `https://www.itabashisekizai.com` | `https://www.itabashisekizai.com` |
| `PUBLIC_GA_MEASUREMENT_ID` | GA4 を Partytown 経由で読み込む | `G-XXXXXXXXXX` |
| `PUBLIC_GBP_URL` | 任意。設定時だけ構造化データの `sameAs` にGoogleビジネスプロフィールURLを追加 | `https://maps.app.goo.gl/...` |
...
1. Cloudflare Pages の環境変数に `PUBLIC_SITE_URL=https://www.itabashisekizai.com` を設定
2. GA4 プロパティを作成し、測定IDを `PUBLIC_GA_MEASUREMENT_ID` に設定
3. Google Search Console に登録し、`https://www.itabashisekizai.com/sitemap-index.xml` を送信
4. Googleビジネスプロフィール確認後、必要なら `PUBLIC_GBP_URL` を設定
